...
import (
	"github.com/elastic/beats/libbeat/kvstore/backend"
)
...
func (tx *transaction) Count() (int, error) {
	...
	if tx.IsReadonly() || len(tx.state.bins) == 0 {
		return tx.countStored(), nil
	} else {
		return tx.countTx(), nil
	}
}

func (tx *transaction) countStored() int {
	return tx.store.mem.tbl.Len()
}

func (tx *transaction) countTx() int {
	n := 0

	// count in bins
	for hash, bin := range tx.store.mem.tbl.bins {
		line := tx.state.bins[hash]
		n += countActiveHashLine(hash, bin, line)
	}

	// cound cached lines not in the store:
	for _, line := range tx.state.bins {
		n += len(line)
	}
...
func countActiveHashLine(hash uint64, bin bin, line txCacheLine) int {
	n := 0
	iterActiveHashLine(
		func(_ cacheEntryRef) error { n++; return nil },
		func(_ valueRef) error { n++; return nil },
		hash, bin, line,
	)
	return n
}
